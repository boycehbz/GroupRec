...
class DemoData_SMPL(DemoData):
    def __init__(self, train=True, dtype=torch.float32, data_folder='', name='', smpl=None):
        super(DemoData_SMPL, self).__init__(train=train, dtype=dtype, data_folder=data_folder, name=name, smpl=smpl)

        self.max_people = 8
        self.use_gt_intri = False
    ...
    def create_data(self, index=0):
        ...
        norm_imgs = torch.zeros((batch_size, self.max_people, 3, 256, 192)).float()
        ...
        for batch_idx in range(batch_size):
            ...
            valid[batch_idx][:num_people] = 1.

            intris = [intri] * num_people
            cliff_data = prepare_cliff(img, boxes, intris=intris)

            norm_imgs[batch_idx][:num_people] = cliff_data['norm_img']
            centers[batch_idx][:num_people] = cliff_data['center']
            scales[batch_idx][:num_people] = cliff_data['scale']
            crop_uls[batch_idx][:num_people] = cliff_data['crop_ul']
            crop_brs[batch_idx][:num_people] = cliff_data['crop_br']
            img_hs[batch_idx][:num_people] = cliff_data['img_h']
            img_ws[batch_idx][:num_people] = cliff_data['img_w']
            focal_lengthes[batch_idx][:num_people] = cliff_data['focal_length']
